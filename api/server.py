...
@app.route('/api/hooks/stop', methods=['POST'])
def hook_stop():
    ...
    _hook_last_seen[name] = time.time()

    # Update tracked session file if we got a new path
    if transcript_path and transcript_path != _worker_sessions.get(name):
        _worker_sessions[name] = transcript_path

    # Check idle state in background thread (avoids blocking Claude)
    filepath = transcript_path or _worker_sessions.get(name)
    def _check_idle_bg():
...
@app.route('/api/hooks/prompt', methods=['POST'])
def hook_prompt():
...
# Global idle state for all workers (written by _bg_idle_monitor + hooks, read by _bg_workers_monitor)
_idle_state = {}        # name -> bool (True = idle)
_idle_state_lock = threading.Lock()

# Hook-based idle detection: track when hooks last reported state
_hook_last_seen = {}    # worker name -> timestamp of last hook event
            ...
            try:
                ...
                for name in active_names:
                    try:
                        ...
                        last_hook = _hook_last_seen.get(name, 0)
                        if now - last_hook < 30:
                            continue
                ...
                for name in list(_hook_last_seen):
                    if name not in active_names:
                        del _hook_last_seen[name]
                with _idle_state_lock:
                    for name in list(_idle_state):
                        if name not in active_names:
                            del _idle_state[name]
